...
// ======================== Setear modal de préstamo =========================================
    ...
    $('.modal').on('click', '#btn-prestar-ambiente', function(event) {
        ...
        var $form_prestamo = $('#form-prestamo');
            $classroom_id  = $form_prestamo.find('#id_ambiente').val(),
            $prestado_en   = $form_prestamo.find('input[name=prestado_en]').val(),
            $id_instructor  = $form_prestamo.find('input[name=instructor_id]').val(),
            $token = $form_prestamo.find('input[name=_token]').val();
        if($id_instructor > 0) {
...
// ======================== Setear modal de entrega ========================================
    ...
    $('body').on('click', '.form-truncate-ficha', function(e){
        ...
        $modalTrun.modal({ backdrop: 'static', keyboard: false })
    ...
    $('.modal').on('hidden.bs.modal', function (e) {
        $(this)
            .find("input[type=search], input[name=id], input[name=prestado_en], input[id=nomInstructor], textarea[name=novedad], select")
               .val('')
               .end()
            .find("input[type=checkbox], input[type=radio]")
               .prop("checked", "")
               .end()
            .find("#resultado_instructor")
                .children()
                .remove();
    });
});
